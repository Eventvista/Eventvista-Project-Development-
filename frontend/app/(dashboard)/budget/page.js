...
/**
 * @file frontend/app/(dashboard)/budget/page.js
 * @description Dynamic ledger analysis page utilizing derived URL query parameters
 * to coordinate backend financial reports in real-time.
 */
...
export default function BudgetPage() {
  ...
  // Synchronously derive active event context directly from the router SSOT
  ...
  // Coordinate data-fetching cycles directly linked to URL query state changes
  useEffect(() => {
    if (activeEventId) {
  ...
  const handleAddExpense = async (e) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) {
        // Force immediate sync update to guarantee client consistency
        fetchBudgetDetails(activeEventId);
