  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-center gap-3 rounded-xl border border-neutral-200
                 ...
                 shadow-sm transition-all duration-150 hover:shadow-md hover:border-neutral-300 
                 active:scale-[0.98] focus:ring-2 focus:ring-purple-100 disabled:opacity-60 disabled:cursor-not-allowed"
    >
      {loading ? (
...
export function RoleSelectionModal({ open, onSelect }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/50 backdrop-blur-sm px-4" role="dialog" aria-modal="true">
      <div className="w-full max-w-md rounded-2xl bg-white p-7 shadow-xl animate-in fade-in zoom-in-95 duration-150">
          ...
          {roles.map((role) => {
            ...
            return (
              <button
                ...
                className={`text-left rounded-xl border-2 p-4 transition-all duration-150 focus:outline-none
                  ${selected ? "border-purple-600 bg-purple-50/50" : "border-neutral-200 hover:border-neutral-300"}`}
              >
        ...
        <button
          ...
          className="mt-6 w-full rounded-xl bg-purple-600 px-5 py-3 font-medium text-white transition-all duration-150 
                     hover:bg-purple-700 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {confirming ? "Saving Profile…" : "Continue"}
        </button>
      </div>
    </div>
  );
